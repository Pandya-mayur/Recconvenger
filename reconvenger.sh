...
╚═╝  ╚═╝╚══════╝ ╚═════╝ ╚═════╝ ╚═╝  ╚═══╝  ╚═══╝  ╚══════╝╚═╝  ╚═══╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝
                            ...
                            V0.1.2 (beta)
                            Coded By TheLinuxBoys with Love <3 ❤
    "$reset""
...
fun_help () {
  ...
  echo -e "   Streamline your recon and vulnerability detection process with reconvenger,"
  ...
  echo -e "   reconvenger [MODE] [FLAGS]"
  echo -e "   reconvenger -m EXP -d target.com -c /path/to/config.yaml"
  echo -e "\n"

  echo -e "${lyellow}[MODES:]${reset}"
  echo -e "${yellow}    ['-m'/'--mode']${reset}"
  echo -e "         Available Options for MODE: "
  echo -e "         SUB | sub | SUBDOMAIN | subdomain           Run reconvenger in SUBDOMAIN ENUMERATION mode"
  echo -e "         URL | url                                   Run reconvenger in URL ENUMERATION mode"
  echo -e "         EXP | exp | EXPLOIT | exploit               Run reconvenger in Full Exploitation mode"
  echo -e "\n"
  ...
  echo -e "     Run reconvenger in full Exploitation mode"
  echo -e "${lyellow}         reconvenger -m EXP -d target.com${reset}"

  echo -e "\n"
  echo -e "     Use your own CONFIG file for subfinder"
  echo -e "${lyellow}         reconvenger -m EXP -d target.com -c /path/to/config.yaml${reset}"
  echo -e "\n"

  echo -e "     Run reconvenger in SUBDOMAIN ENUMERATION mode"
  echo -e "${lyellow}         reconvenger -m SUB -d target.com${reset}"
  echo -e "\n"

  echo -e "     Run reconvenger in URL ENUMERATION mode"
  echo -e "${lyellow}         reconvenger -m URL -d target.com${reset}"
  echo -e "\n"
}
